...
// import { fetchAdminBookings, fetchUserBookings } from '../../store/slices/bookingSlice';
                ...
                <div className="flex space-x-2">
                  {Array.from({ length: usersTotalPages }, (_, i) => i + 1).map((page) => (
                    <button
                      ...
                      className={`px-3 py-1 rounded-lg ${page === usersCurrentPage
      ...
      <div className="p-6 border-b border-gray-200">
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            ...
            <div>
              <h2 className="text-xl font-semibold text-gray-900">Farmhouse Management</h2>
              <p className="text-gray-600">Manage all farmhouse listings</p>
            </div>
          </div>
          <div className="text-sm text-gray-500">
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-sm text-gray-600">
                            <div>
                              ...
                              <p>{farmhouse.address?.city}, {farmhouse.address?.state}</p>
                            </div>
                            <div>
                      ...
                      <div className="flex flex-col sm:flex-row lg:flex-col gap-2 mt-4 lg:mt-0 lg:ml-4 lg:w-32">
                        <button
                          ...
                          className="px-3 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-sm font-medium flex items-center justify-center"
                        >
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <p className="text-gray-600 mb-4">
              ...
              This action cannot be undone and will permanently remove the farmhouse and all associated data.
            </p>
            <div className="flex justify-end space-x-3">
              ...
              <button
                onClick={() => handleDeleteFarmhouse(deleteConfirm.id, deleteConfirm.name)}
...
const BookingManagement = ({ onClose }) => {
  ...
  // ✅ FIXED: Remove $ symbol from currency formatting
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-gray-600">All completed bookings across the platform</p>
            </div>
          </div>
          <div className="text-sm text-gray-500">
            ...
            {bookings.length > 0 && (
              <div className="mt-6 p-4 bg-blue-50 rounded-lg max-w-2xl mx-auto text-left">
                ...
                <div className="text-sm text-blue-700 space-y-1">
                  <p>• Total bookings loaded: <strong>{bookings.length}</strong></p>
                  <p>• Completed bookings found: <strong>{completedBookings.length}</strong></p>
                  <p>• Booking statuses in system:</p>
                  <div className="ml-4">
                ...
                <div className="flex items-start space-x-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-gray-600">
                      ...
                      <div>
                        <span className="font-medium text-gray-700">Contact:</span>
                        {/* ✅ IMPROVED: Multiple fallback options for phone */}
                        <p className="text-sm">
                          {booking.contactInfo?.phone && booking.contactInfo.phone !== '0000'
                            ? booking.contactInfo.phone
                            : booking.user?.phone && booking.user.phone !== '0000'
                              ? booking.user.phone
                              : booking.user?.phone // This might be '0000'
                                ? 'Not provided'
                                : 'Not provided'
                          }
                        </p>
                        {/* ✅ ADD: Debug info for phone numbers */}
                        {booking.contactInfo?.phone === '0000' && (
                          <p className="text-xs text-red-500">⚠️ Contact phone shows default value</p>
                        )}
                      </div>
                    </div>
                    ...
                    {/* Booking ID */}
                    <div className="mt-2">
                      <span className="font-medium text-gray-700 text-sm">Booking ID: </span>
                      <span className="text-sm text-gray-600 font-mono">{booking._id}</span>
                    </div>
                    ...
                    {booking.review ? (
                      <div className="mt-3">
                        <span className="font-medium text-gray-700 text-sm">Guest Review: </span>
                        <div className="flex items-center">
                      ...
                      <div className="mt-3">
                        <span className="text-sm text-gray-500">No review submitted</span>
                      </div>
                    )}
                  </div>
                </div>
...
const AdminDashboard = () => {
  ...
  const [activeManagement, setActiveManagement] = useState(null); // 'users', 'farmhouses', 'bookings'
  ...
  // Fetch pending farmhouses when approvals tab is active
  useEffect(() => {
  ...
  // Refresh stats when farmhouses are approved/rejected
  useEffect(() => {
  ...
  const handleCloseManagement = () => {
    ...
    dispatch(fetchAdminStatistics()); // Refresh stats when returning to overview
  };

  const handleApprove = async (farmhouseId) => {
    try {
      console.log('🔄 Attempting to approve farmhouse with ID:', farmhouseId);

      if (!farmhouseId || typeof farmhouseId !== 'string') {
        alert('❌ Invalid farmhouse ID: ID is missing or not a string');
        return;
      }

      const cleanId = farmhouseId.replace(/[^a-fA-F0-9]/g, '');
      if (cleanId.length !== 24) {
        alert(`❌ Invalid farmhouse ID format. Expected 24 characters, got ${cleanId.length}`);
        return;
      }

      console.log('📤 Dispatching approve action with cleaned ID:', cleanId);

      const result = await dispatch(approveFarmhouse(farmhouseId)).unwrap();

      console.log('✅ Farmhouse approved successfully:', result);
      ...
      console.error('❌ Failed to approve farmhouse - Full error:', error);
      const errorMessage = error.message ||
        error.response?.data?.message ||
        error.response?.data?.details ||
        'Please try again';

      alert(`❌ Failed to approve farmhouse: ${errorMessage}`);
    }
  };
  ...
  const handleReject = async () => {
    ...
    try {
      console.log('🔄 Attempting to reject farmhouse:', selectedFarmhouse._id);

      if (!selectedFarmhouse._id || typeof selectedFarmhouse._id !== 'string') {
        alert('❌ Invalid farmhouse ID: ID is missing or not a string');
        return;
      }

      const cleanId = selectedFarmhouse._id.replace(/[^a-fA-F0-9]/g, '');
      if (cleanId.length !== 24) {
        alert(`❌ Invalid farmhouse ID format. Expected 24 characters, got ${cleanId.length}`);
        return;
      }

      const result = await dispatch(rejectFarmhouse({
        id: selectedFarmhouse._id,
        rejectionReason
      })).unwrap();

      console.log('✅ Farmhouse rejected successfully:', result);
      ...
      console.error('❌ Failed to reject farmhouse - Full error:', error);
      const errorMessage = error.message ||
        error.response?.data?.message ||
        error.response?.data?.details ||
        'Please try again';

      alert(`❌ Failed to reject farmhouse: ${errorMessage}`);
    }
  };
  ...
  if (activeManagement === 'users') {
    return <UserManagement onClose={handleCloseManagement} />;
  }

  if (activeManagement === 'farmhouses') {
    return <FarmhouseManagement onClose={handleCloseManagement} />;
  }

  if (activeManagement === 'bookings') {
    return <BookingManagement onClose={handleCloseManagement} />;
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-600 mt-2">Manage platform statistics and farmhouse approvals</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          {/* Total Farmhouses - Clickable */}
          <div
            onClick={() => handleStatCardClick('farmhouses')}
